...
data = pd.read_csv('../inputs/application_train.csv')
test = pd.read_csv('../inputs/application_test.csv')
prev = pd.read_csv('../inputs/previous_application.csv')
buro = pd.read_csv('../inputs/bureau.csv')
buro_balance = pd.read_csv('../inputs/bureau_balance.csv')
credit_card  = pd.read_csv('../inputs/credit_card_balance.csv')
POS_CASH  = pd.read_csv('../inputs/POS_CASH_balance.csv')
payments = pd.read_csv('../inputs/installments_payments.csv')
lgbm_submission = pd.read_csv('../inputs/sample_submission.csv')
...
## store train_matrix before call model training 
# app_train_domain.to_csv('final_train_matrix.csv')
# app_test_domain.to_csv('final_test_matrix.csv')
...
def model(features, test_features, encoding = 'ohe', n_folds =5 ):
    ...
    for train_indices, valid_indices in k_fold.split(features):
        ...
        # Create the model
        model = lgb.LGBMClassifier(n_estimators=10000, nthread=8, objective = 'binary',
                                   metric='auc',
                                   boosting_type="gbdt",
                                   class_weight = 'balanced', learning_rate = 0.05, 
                                   reg_alpha = 0, reg_lambda = 0, 
                                   n_jobs = -1, random_state = 50,num_leaves=32,colsample_bytree=0.7,subsample=.8715623,
                                  max_depth=6,min_split_gain=.0222415,min_child_weight=30,max_bin=200,num_boost_round=3000,min_data_in_leaf=400,bagging_fraction=0.5,bagging_freq=10)
